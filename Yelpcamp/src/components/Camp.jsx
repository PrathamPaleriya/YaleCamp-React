  ...
  return (
    <div className='grid grid-cols-1 lg:grid-cols-3 px-8 lg:px-20 my-10 gap-10'>
      ...
      <div className='border-2 border-cream-outline rounded-md w-full p-10 lg:col-span-2 lg:order-2'>
        <div>
          <img src={imageURL} alt="" className='object-cover object-center w-full lg:h-[550px]' />
          <div className='flex justify-between items-center'>
            <h1 className='my-5 text-md md:text-xl lg:text:2xl font-bold'>{data.Title}</h1>
            <p className='text-sm md:text-lg lg:text-xl'>{`INR: ${data.rate}/night`}</p>
          </div>
          <p className='md:text-lg lg:text-xl text-cream-dark'>{data.description}</p>
        </div>
      </div>
    </div>
  )
